# NutriAI App V1.0 (Expo + React Native + TypeScript)

## V1.0: Anamnese conversacional

Fluxo MLP implementado:

- Primeiro acesso (após login): `Welcome -> Chat de Anamnese -> Resultado -> Home`
- Próximos acessos: `Login -> Home`
- Integração com API serverless:
  - `POST /v1/anamnese/start`
  - `POST /v1/anamnese/answer`
  - `GET /health`

Quando a anamnese conclui, o app salva estado local para pular onboarding nos próximos logins.
...
```env
API_URL=https://SEU_API_ID.execute-api.us-east-1.amazonaws.com
APP_NAME=NutriAI
```
...
## Como testar o fluxo completo V1.0 no Expo Go

1. Garanta que `API_URL` está apontando para seu backend.
2. Rode `npm start`.
3. Abra no Expo Go e faça login.
4. Complete o fluxo: Welcome -> Chat -> Resultado -> Home.
5. Feche e abra o app: o onboarding deve ser pulado após concluir.
6. Teste offline: desligue internet e tente iniciar/continuar a anamnese.
7. Teste API fora do ar (URL inválida) para validar tratamento de erro amigável.
